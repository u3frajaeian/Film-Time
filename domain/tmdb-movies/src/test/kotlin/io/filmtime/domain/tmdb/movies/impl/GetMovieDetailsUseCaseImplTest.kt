...
class GetMovieDetailsUseCaseImplTest{
  ...
  @Test
  fun `call getMovie and get similar as well`() = runBlocking {
    val movieId = 1
    ...
    `when`(repository.getMovieDetails(movieId)).thenReturn(expectedResult)

    val result = getMoviesDetailUseCase.invoke(movieId)
